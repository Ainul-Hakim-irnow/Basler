        ...
        try:
            ...
            while self._running and cam.IsGrabbing():
                ...
                if inference_enabled_local and model_local:
                    ...
                    # labels = [f"{class_names[i.item()]} {s:.2f}" for i, s in zip(labels_idx, scores)]
                    final_labels = [f"{class_names[i.item()]} {s:.2f}" for i, s in zip(filtered_labels_idx, filtered_scores)]
...
class MainWindow(QMainWindow):
    ...
    def update_resolution(self, text):
        """Updates the target processing height and sends it to all running threads."""
